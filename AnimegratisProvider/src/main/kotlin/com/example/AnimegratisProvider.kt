...
class AnimeGratisProvider : MainAPI() {
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        ...
        val ssrScript = document.selectFirst("script#ssr-init")?.data()
        val home = if (!ssrScript.isNullOrEmpty()) {
            parseSsrJson(
                ssrScript,
                request.name
            )
        } else {
            document.select("a.inline-block").mapNotNull { it.toSearchResult() }
        }
    ...
    private fun parseSsrJson(json: String, category: String): List<SearchResponse> {
        ...
        return try {
            ...
            for (i in 0 until items.length()) {
                ...
                val image = item.optString("im", "")
                val type = item.optString("ty", "")
                val href = "$mainUrl/anime/$slug-anime"
                val tvType = when {
                    type.contains("anime", ignoreCase = true) -> TvType.Anime
                    type.contains("donghua", ignoreCase = true) -> TvType.Anime
                    type.contains("pelicula", ignoreCase = true) || type.contains("movie", ignoreCase = true) -> TvType.AnimeMovie
                    else -> TvType.Anime
                }
                results.add(
    ...
    private fun Element.toSearchResult(): SearchResponse {
        val title = this.select("h3, .title").text().trim()
    ...
    override suspend fun search(query: String): List<SearchResponse> {
        ...
        val ssrScript = document.selectFirst("script#ssr-init")?.data()
        return if (!ssrScript.isNullOrEmpty()) {
            parseSsrJson(ssrScript, "Búsqueda")
        } else {
            document.select("a.inline-block").mapNotNull { it.toSearchResult() }
        }
    }
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        val poster = document.selectFirst("img[alt*='Poster'], img.cover")?.attr("src")
            ?: document.selectFirst("img.rounded-lg")?.attr("src") ?: ""

        val description = document.selectFirst("div.description, .synopsis p")?.text()
            ?: document.selectFirst("p.line-clamp-4")?.text()

        val tags = document.select("div.tags a, .genres a").map { it.text() }

        val statusText = document.selectFirst("span.status, .status")?.text()
        ...
        val yearText = document.selectFirst("span.year, .year")?.text()
        val year = yearText?.toIntOrNull()

        val recommendations = document.select("section.recommendations article, .related a").mapNotNull {
            val recTitle = it.select("h3, .title").text().trim()
        ...
        document.select("#episodes-grid a, .episodes-list a").forEach {
            val epHref = fixUrl(it.attr("href"))
            val epText = it.select("span, .ep-number").text().trim()
            val epNum = Regex("(\\d+)").find(epText)?.groupValues?.get(1)?.toIntOrNull()
            val epPoster = fixUrlNull(it.select("img").attr("src"))
            episodes.add(
                newEpisode(epHref) {
    ...
    override suspend fun loadLinks(
        ...
        Log.d("AnimeGratis", "loadLinks iniciado con URL: $data")
        val document = try {
            app.get(data).document
        } catch (e: Exception) {
            Log.e("AnimeGratis", "Error al cargar documento: ${e.message}")
            return false
        }
        ...
        document.select("button.server-btn").forEach { btn ->
            ...
            if (serverUrl.isNotBlank()) {
                ...
                Log.d("AnimeGratis", "Servidor encontrado: $displayName -> $serverUrl")
                try {
                    loadExtractor(serverUrl, referer = mainUrl, subtitleCallback, callback)
                } catch (e: Exception) {
                    Log.e("AnimeGratis", "Error loadExtractor $serverName: ${e.message}")
                }
            }
        }

        document.select("iframe#videoFrame").map { it.attr("src") }.forEach { iframeUrl ->
            if (iframeUrl.isNotBlank()) {
                Log.d("AnimeGratis", "iframe principal: $iframeUrl")
                loadExtractor(fixUrl(iframeUrl), referer = mainUrl, subtitleCallback, callback)
            }
        }
