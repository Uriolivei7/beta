...
class Tvporinternet2Provider : MainAPI() {
    ...
    private val peliculasSeriesCat = setOf(
        ...
        "USA", "Canal Sony", "Golden Plus", "Golden Edge", "Golden Premier", "FM Hot Movies"
    )
    ...
    private val entretenimientoCat = setOf(
        ...
        "Azteca Uno", "Antena 3", "DW", "Telemundo 51", "Telemundo Puerto Rico"
    )
    ...
    private val deportesCat = setOf(
        ...
        "Azteca Deportes", "Liga 1", "Sky Sports", "VIX TUDN", "Liga 1 Max", "Fox Sports Premium",
        "Sky Sports La Liga", "Movistar La Liga", "Movistar Liga De Campeones", "Bein Sports Xtra",
        "Directv Sports 2", "Directv Sports Plus", "ESPN Premium", "ESPN 2", "ESPN 3", "ESPN 4",
        "ESPN 5", "ESPN 6", "ESPN 7", "ESPN Mexico", "ESPN 2 Mexico", "ESPN 3 Mexico",
        "ESPN 4 Mexico", "ESPN 5 Mexico", "Fox Sports 2", "Fox Sports 3", "Fox Sports Mexico",
        "Fox Sports 2 Mexico", "Fox Sports 3 Mexico", "TyC Sports", "Mundo Latam"
    )
    ...
    private suspend fun safeAppGet(
        ...
        for (attempt in 1..2) {
            try {
                ...
                Log.e("Tvporinternet2", "safeAppGet error (intento $attempt): ${e::class.simpleName} - ${e.message}")
            }
        }
        return null
    }
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse? {
        val html = safeAppGet(mainUrl) ?: run { Log.e("Tvporinternet2", "getMainPage: failed to get HTML"); return null }
        val channels = extractChannelsFromHtml(html)
        if (channels.isEmpty()) Log.w("Tvporinternet2", "getMainPage: 0 canales extraídos (selectors changed?)")
        ...
        Log.d("Tvporinternet2", "getMainPage: ${channelResults.size} canales")
    ...
    private fun extractChannelsFromHtml(html: String): List<Triple<String, String, String>> {
        ...
        val selectors = listOf("a.channel-card", "a[href*=-en-vivo-por-internet]")
    ...
    override suspend fun search(query: String): List<SearchResponse> {
        Log.d("Tvporinternet2", "Search: query='$query'")
        val html = safeAppGet(mainUrl) ?: run { Log.e("Tvporinternet2", "Search: failed to get HTML"); return emptyList() }
        val channels = extractChannelsFromHtml(html)
        if (channels.isEmpty()) Log.w("Tvporinternet2", "Search: 0 canales (getMainPage also failing?)")
        ...
        Log.d("Tvporinternet2", "Search: ${matched.size} resultados para '$query'")
    ...
    override suspend fun load(url: String): LoadResponse? {
        val html = safeAppGet(url) ?: run { Log.e("Tvporinternet2", "load: failed to get HTML $url"); return null }
        val doc = Jsoup.parse(html)
        val title = doc.selectFirst("h1.font-bold")?.text()
            ?: doc.selectFirst("h1")?.text()
            ?: doc.selectFirst("meta[property='og:title']")?.attr("content")
            ?: doc.selectFirst("title")?.text()
            ?: run { Log.w("Tvporinternet2", "load: title not found $url"); "Canal Desconocido" }
        ...
        if (poster.isBlank()) Log.w("Tvporinternet2", "load: poster not found for $cleanTitle")
        Log.d("Tvporinternet2", "load: $cleanTitle")
    ...
    override suspend fun loadLinks(
        ...
        Log.d("Tvporinternet2", "Logs: Cargando URL base -> $targetUrl")
        ...
        try {
            ...
            val mainPageResponse = withTimeoutOrNull(20000L) { app.get(targetUrl, headers = mainHeaders, interceptor = cfKiller) }
                ?: run {
                    Log.w("Tvporinternet2", "Logs: Timeout al cargar página principal")
                    return false
                }
            ...
            if (directM3u8 != null) {
                Log.d("Tvporinternet2", "Logs: M3U8 encontrado directamente en página principal: $directM3u8")
                callback(
                    newExtractorLink(
                        source = this.name,
            ...
            Log.d("Tvporinternet2", "Logs: optionLinks raw=${optionLinks.joinToString()}")
            ...
            Log.d("Tvporinternet2", "Logs: Opciones detectadas: ${finalLinks.size}")
            ...
            Log.d("Tvporinternet2", "channelPageResp: ${channelPageResp?.code ?: "null"} cookies: ${channelCookies.size}")
            if (channelCookies.isEmpty()) {
                Log.w("Tvporinternet2", "channel page no cookies, will try live/core.php")
            }
            ...
            Log.e("Tvporinternet2", "Logs: Error crítico: ${e.message}")
            return false
        }
    }
    ...
    private suspend fun tryLoadOption(
        ...
        return try {
            Log.d("Tvporinternet2", "Logs: Probando Opción ${displayIndex + 1} -> $playerUrl")
                ...
                withTimeout(requestTimeout) {
                    ...
                    val playerCookies = if (playerResponse.cookies.isNotEmpty()) {
                        ...
                        if (setCookieHeaders.isNotEmpty()) {
                            Log.d("Tvporinternet2", "Found Set-Cookie headers: ${setCookieHeaders.size}")
                    ...
                    Log.d("Tvporinternet2", "playerResponse: ${playerResponse.code} cookies: ${playerResponse.cookies.size} setCookieHeaders: ${playerResponse.headers?.names()?.filter { it.lowercase() == "set-cookie" }?.size ?: 0} -> combined: ${playerCookies.size}")
                    ...
                    if (pageTitle.contains("pagina no encontrada") || pageTitle.contains("página no encontrada") || pageTitle.contains("404")) {
                        Log.w("Tvporinternet2", "Logs: Opción ${displayIndex + 1} fail rápido - página no encontrada")
                    ...
                    val internalIframe = Regex("""<iframe[^>]+src=["']([^"']+)["']""", RegexOption.IGNORE_CASE).find(playerHtml)?.groupValues?.get(1)
                        ?.replace("&", "&")
                    Log.d("Tvporinternet2", "internalIframe detected: ${internalIframe ?: "null"}")

                    if (internalIframe == null || internalIframe.isBlank()) {
                        Log.w("Tvporinternet2", "No iframe found in playerHtml len=${playerHtml.length}")
                        return@withTimeout false
                    }
                    ...
                    if (iframeHtml == null && internalIframe.contains("regionales.saohgdassregions")) {
                        ...
                        Log.d("Tvporinternet2", "Trying alt domain: $altUrl")
                        val altHtml = tryIframeDomain(altUrl)
                        if (altHtml != null) {
                            Log.d("Tvporinternet2", "Alt domain succeeded")
                            iframeHtml = altHtml
                        }
                    }

                    if (iframeHtml == null) {
                        Log.w("Tvporinternet2", "iframe request failed on all domains url=$iframeUrl")
                        return@withTimeout false
                    }
                    Log.d("Tvporinternet2", "iframeHtml len=${iframeHtml.length}")
                    ...
                    val playlistUrl = Regex("""var\s+src\s*=\s*["']([^"']*playlist\.php[^"']*)["']""", RegexOption.IGNORE_CASE)
                        ...
                        ?.replace("\\/", "/")
                    if (playlistUrl == null) {
                        Log.w("Tvporinternet2", "playlist.php URL not found in iframeHtml snippet=${iframeHtml.take(500)}")
                        return@withTimeout false
                    }
                    Log.d("Tvporinternet2", "playlist.php URL: $playlistUrl")
                    ...
                    if (playlistResp == null || !playlistResp.isSuccessful) {
                        Log.w("Tvporinternet2", "playlist.php request failed: ${playlistResp?.code ?: "timeout/null"} url=$playlistUrl")
                        return@withTimeout false
                    }
                    val m3u8Content = playlistResp.text
                    Log.d("Tvporinternet2", "playlist.php response len=${m3u8Content.length} isM3U8=${m3u8Content.contains("#EXTM3U")}")

                    // La respuesta de playlist.php YA ES el m3u8
                    if (!m3u8Content.contains("#EXTM3U")) {
                        Log.w("Tvporinternet2", "playlist.php no devolvió m3u8 válido snippet=${m3u8Content.take(200)}")
                        return@withTimeout false
                    }
                    ...
                    if (finalM3u8 == null && internalIframe != null) {
                        ...
                        if (altUrl != null) {
                            ...
                            if (altResp != null && altResp.isSuccessful) {
                                ...
                                Log.d("Tvporinternet2", "Alt domain extractM3u8: ${altM3u8 ?: "null"}")
                            } else {
                                Log.w("Tvporinternet2", "Alt domain request failed: ${altResp?.code ?: "null"}")
                            }
                        }
                    }

                    if (!finalM3u8.isNullOrEmpty()) {
                        Log.d("Tvporinternet2", "Logs: ¡Éxito! M3U8: $finalM3u8")

                        val iframeDomain = try { URL(internalIframe ?: playerUrl).host } catch (_: Exception) { "" }
                        val streamingHeaders = mapOf(
                            "User-Agent" to "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36",
                            "Origin" to if (iframeDomain.isNotBlank()) "https://$iframeDomain" else "https://www.tvporinternet2.com",
                            "Referer" to if (internalIframe != null) fixUrl(internalIframe) else playerUrl
                        )
                        Log.d("Tvporinternet2", "Logs: streamingHeaders=$streamingHeaders")

                        callback(
                            newExtractorLink(
                                source = this@Tvporinternet2Provider.name,
                                name = "${this@Tvporinternet2Provider.name} - Opción ${displayIndex + 1}",
                                url = finalM3u8,
                                type = ExtractorLinkType.M3U8
                            ) {
                        ...
                        Log.w("Tvporinternet2", "Logs: Falló Opción ${displayIndex + 1} - no se encontró M3U8")
                        return@withTimeout false
                    }
                }
            } catch (e: TimeoutCancellationException) {
                Log.w("Tvporinternet2", "Logs: Opción ${displayIndex + 1} timeout - ${requestTimeout}ms")
                false
            }
        } catch (e: CancellationException) {
            throw e
        } catch (e: Exception) {
            Log.e("Tvporinternet2", "Logs: Error opción ${displayIndex + 1}: ${e.message}")
            false
        }
    }
