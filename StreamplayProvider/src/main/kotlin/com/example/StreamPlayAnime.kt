...
private fun <T> Resource<T>.safeGetOrThrow(): T {
    return when (this) {
        ...
        is Failure -> throw Exception("API Request Failed: ${this.errorString}")
        is Loading -> throw Exception("API Resource is still loading.")
    }
}
...
class StreamPlayAnime : MainAPI() {
    ...
    data class AllLibraryLists(val allLibraryLists: List<LibraryList>)
    data class LibraryList(val name: String, val items: List<SearchResponse>)
}